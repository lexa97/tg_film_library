...
|-----------|------------|
| Язык | Python 3.10-3.14 (рекомендуется 3.13) |
| Фреймворк бота | aiogram 3.24+ |
| БД | PostgreSQL 14+ |
| Инициализация БД | initdb.py (SQLAlchemy create_all) |
...
1. **Среда и каркас:** репозиторий, venv (Python 3.13), зависимости (aiogram, asyncpg или SQLAlchemy 2, pydantic-settings), .env, структура папок (handlers, services, models, db).
2. **БД:** модели SQLAlchemy и скрипт initdb.py для инициализации, таблицы по п. 7.
