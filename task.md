...
- Срок: ориентир 2–4 недели при частичной занятости.
...
- Участники групп: связь user_id ↔ group_id, роль (admin / member). У одной группы один админ (создатель); при желании позже — несколько админов.
...
- **OMDb:** бесплатный лимит запросов, простой JSON; мультиязычность ограничена. Документация: https://www.omdbapi.com/ .
- **Кинопоиск:** неофициальные API/парсинг; при необходимости — отдельный модуль с учётом правил использования.

Рекомендация для MVP: TMDB как основной источник (учёт разных языков названий и переводов). В коде — абстракция «поисковик» (интерфейс), одна реализация под TMDB; при расширении — вторая реализация (например, OMDb) без смены сценариев бота.
...
- **Лимиты API:** TMDB — соблюдать лимиты запросов и кэшировать при необходимости.
...
- Код на Python (aiogram 3, async), PostgreSQL, описание запуска в README и при необходимости в task.md.

После ответов на вопросы из раздела 3 можно зафиксировать их в этом файле и переходить к реализации по этапам из раздела 9.
